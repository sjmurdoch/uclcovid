...
**There is a 50-hour hole in the snapshot record.** Nothing was fetched between `covid-2021-08-21T08-34-02` and `covid-2021-08-23T10-34-02`; 22 August 2021 has no entries at all, not even deduplicated ones in `manifest-sha256.txt.gz`. This coincides exactly with the `.git` corruption described in `PROVENANCE.md`, and was an outage of the machine rather than of the scraper. No published data was lost — the gap fell across a weekend, when UCL was not publishing, and the cumulative totals reconcile across it — but anyone reasoning from *fetch* times rather than reported dates should know about it.
